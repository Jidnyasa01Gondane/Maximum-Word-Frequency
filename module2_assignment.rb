...
class Solution
  ...
  def analyze_file()
    ...
    File.foreach('test.txt') do |content|
      lineAnalyzer = LineAnalyzer.new(content,i += 1)
      @analyzers << lineAnalyzer
    end
    #@analyzers.each { |e| puts e.highest_wf_words}
    #@analyzers.each { |e| puts e.highest_wf_count }
  end
  ...
  def print_highest_word_frequency_across_lines()
    ...
    @highest_count_words_across_lines.each{ |x| puts "#{x.highest_wf_words} (appears in line_number#{x.line_number})" }
  end
end